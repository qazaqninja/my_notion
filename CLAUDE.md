...
9. 🚧 **WYSIWYG editing via super_editor** — biggest single-feature lift. Source mode + read-only rendered work fine without it. `super_editor: 0.3.0-dev.44` is now in `pubspec.yaml` and `lib/core/markdown/super_editor_serializer.dart` ships the markdown↔MutableDocument bridge skeleton (D1-D5 work). **Still pending:** block-type parity (D6-D15), inline marks parity (D16-D22), interactions parity (D23-D27), cutover (D28-D30). (~24-27 slices remaining from the plan's D-phase)

...
**v1 mostly shipped (M0–M1657+).** Latest commit on `main` is M1657 (NV-03 carry-forward fully resolved — all bare-string route literals migrated to `Routes` constants + `Routes.editor(ulid)` / `Routes.db(dbId)` / `Routes.settingsSection(section)` helpers; 53+ call sites swapped across 6 slices). The original Pick-next queue (items 1/3/4/6/9 from FEATURES.md) is fully landed; subsequent milestones extended through 1,650+ commits adding block-editor depth, database polish, slash-menu extractors (M1100–M1186 series — ~110 source-text extractors), Phase E V2 backend (auth + sync + public + forms + presence + WebSocket CRDT seam), Phase G mobile shell + Phase H presence layer, full WYSIWYG D-fp parity ports (Move-to-Trash + Pull-from-server + Share-via-OS + Find-in-page with Cmd+F/⌘G/⌘⇧G/Esc shortcuts), and `lib/core/routing/routes.dart` typed-route helper module. ~2,800+ Flutter tests across 180 test files + 192 backend tests across 15 test files, both `flutter analyze` and backend `dart analyze` clean.

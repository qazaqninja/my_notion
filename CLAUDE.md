...
**v1 mostly shipped (M0–M1693+).** Latest commit on `main` is M1693 (loop-state advance recording the M1692 FEATURES.md early-section sweep — status counter `M0–M1186+` → `M0–M1690+`, prose `1,180+ milestones` → `1,690+ milestones`, and the public-page-sharing 🔮 → ✅ flip citing Phase E E16/E17/E21/E43/E44 server-side surfaces at `backend/lib/public/{routes,markdown_html}.dart`). The original Pick-next queue (items 1/3/4/6/9 from FEATURES.md) is fully landed; subsequent milestones extended through 1,690+ commits adding block-editor depth, database polish, slash-menu extractors (M1100–M1186 series — ~110 source-text extractors), Phase E V2 backend (auth + sync + public + forms + presence + WebSocket CRDT seam), Phase G mobile shell + Phase H presence layer, full WYSIWYG D-fp parity ports (Move-to-Trash + Pull-from-server + Share-via-OS + Find-in-page with Cmd+F/⌘G/⌘⇧G/Esc shortcuts), full D28-D30a WYSIWYG cutover gateway (Settings → Advanced toggle + GoRoute fork + default-true flip), `lib/core/routing/routes.dart` typed-route helper module, `EditorPreferencesStore` BL-01 abstraction (`lib/features/settings/data/editor_preferences_store.dart`), and a touch-tuned mobile UI pass (slash menu + properties panel hit-targets at 44pt). ~2,800+ Flutter tests across 185 test files + 192 backend tests across 15 test files, both `flutter analyze` and backend `dart analyze` clean.
...
- Editor: hand-rolled `MarkdownRenderer` (read-only) + `TextField` for source on the legacy `/editor/:ulid` route. The beta `/editor-beta/:ulid` route runs full WYSIWYG via `super_editor` 0.3.0-dev.51 (D1-D27 + D-fp parity ports landed). D28-D30 cutover (flip /editor-beta to default /editor) remains the last v1.x lift.
...
9. 🚧 **WYSIWYG editing via super_editor** — biggest single-feature lift. `super_editor: 0.3.0-dev.51` in `pubspec.yaml`. `lib/core/markdown/super_editor_serializer.dart` is the markdown↔MutableDocument bridge. **D1-D22 ✅** (paragraph + headings + lists + todos + code + hr + blockquote + GFM callouts + math + mermaid + GFM pipe tables + image cards + file attachments + bookmarks + sub-page + transclusion + columns + breadcrumb + toc + button fences + bold + inline code + italic + strike + underline + highlight + sub + sup + color + wikilink chip + date pill). **D23 ✅** (slash menu wiring at /editor-beta — M1521-M1545: caret-driven trigger session, DocumentLayout caret-rect lookup, overlay rendering + selection-splice, list/todo conversion, async picker actions). **D24a/c/d ✅** (block reorder Cmd+Shift+Up/Down M1570, duplicate Cmd+D M1574, delete Cmd+Shift+Backspace M1578). **D25 ✅** (multi-block selection M1597-M1607: BlockSelection value object, cubit, overlay, gesture wiring, D24-family ops act on multi-select). **D26 ✅** (inline autoformat M1549-M1564: bold, italic, strike, inline code, highlight, sub, sup). **D27 ✅** (heading conversion Cmd+Opt+1/2/3/0 M1583, list/todo conversion Cmd+Shift+7/8/9 M1591). **D-fp parity ports ✅** (Move-to-Trash M1621, Pull-from-server M1623, Share-via-OS M1625, Find-in-page with Cmd+F/⌘G/⌘⇧G/Esc M1627-M1638). **D28 ✅** (M1662/M1665/M1667: `EditorPreferencesCubit` + Settings → Advanced "Use beta WYSIWYG editor" toggle + `/editor/:ulid` GoRoute forks via `BlocSelector`). **D29 ✅** (M1670: flipped `useBeta` default `false` → `true`; pre-D29 explicit opt-outs preserved). **D30a ✅** (M1673: extracted `buildEditorPageForFork` + 8 unit tests as a pre-deletion safety net). **Still pending:** D30b/c — collapse the `BlocSelector` fork to a direct `EditorBetaPage` builder + delete legacy `EditorPage` + `EditorPreferencesCubit` (deferred until dogfood time accumulates after the D29 default-flip). A chunk of M234-M268 source-mode shortcuts may still be source-mode-only. (~1-2 slices remaining)

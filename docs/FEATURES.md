...
- 🚧 Import from: Evernote, Google Docs, Word, CSV, HTML, Markdown, Trello, Asana, Confluence, Quip, Dropbox Paper, Workflowy, Roam, plain text — CSV → database M29. HTML → page M175 (also works for Quip / Dropbox Paper / Google Docs HTML exports via the generic converter). Markdown + plain text → page M178. OPML / Workflowy outlines → page M189. Roam JSON → multi-page M190. Trello board JSON → database (with board view) M191. Evernote / Word / Asana / Confluence still 📋.

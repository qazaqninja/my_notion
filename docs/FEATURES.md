...
2. **WYSIWYG editing via `super_editor`** — Wire the dependency that's already in `pubspec.yaml`. Build the bridge serializer at `lib/core/markdown/super_editor_serializer.dart`. The renderer (`markdown_renderer.dart`) becomes the read-only fallback. 2–3 days including round-trip audit against the byte-identical contract.
...
5. **Math (KaTeX) + Mermaid** — Add `flutter_math_fork` and a Mermaid renderer (webview fallback acceptable). Parse `$...$` / `$$...$$` and ```mermaid in `markdown_renderer.dart`. 1 day.

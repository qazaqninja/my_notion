...
- ✅ Page icons (emoji, custom upload, Notion gallery) — M26, `lib/shared/widgets/page_icon.dart` renders frontmatter `icon:` (emoji / asset / file / URL). Emoji picker (M62) at `lib/shared/widgets/emoji_picker.dart`. Sidebar row icon (M84) shows the same glyph in db row cells. M253–M259 also surface page emoji icons in every place a page is listed: M253 Workspace tree (via a new `VaultFile.icon` + `SideItem.emojiIcon` slot), M254 Favorites + Recent sidebar lists, M255 command palette page rows (via a new `PageSearchResult.emojiIcon`), M256 right-rail "Linked from" backlink cards (via a new `Backlink.emojiIcon`), M257 standalone `[[ULID]]` sub-page cards in the editor body, M258 database Board cards + Timeline ribbon titles, M259 database Calendar day-list rows (all via a local `_displayTitle` helper that reads the row's `icon:` cell). Asset / URL icons stay invisible across all seven surfaces since a 12–22px row can't render an async-loaded image.
...
All nine transformations live in pure helpers (`duplicateLineAt`, `moveLineUp`, `moveLineDown`, `toggleCommentLine`, `deleteLineAt`, `lineRangeAt`, `continueListAtNewline`, `backspaceListMarker`, `joinLineWithNext`) at `lib/features/editor/domain/source_line_ops.dart` and are unit-tested independently of the widget tree (54 cases).

...
All eight transformations live in pure helpers (`duplicateLineAt`, `moveLineUp`, `moveLineDown`, `toggleCommentLine`, `deleteLineAt`, `lineRangeAt`, `continueListAtNewline`, `backspaceListMarker`) at `lib/features/editor/domain/source_line_ops.dart` and are unit-tested independently of the widget tree (48 cases).

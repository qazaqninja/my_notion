...
- ✅ Page icons (emoji, custom upload, Notion gallery) — M26, `lib/shared/widgets/page_icon.dart` renders frontmatter `icon:` (emoji / asset / file / URL). Emoji picker (M62) at `lib/shared/widgets/emoji_picker.dart`. Sidebar row icon (M84) shows the same glyph in db row cells. M253–M257 also surface page emoji icons in every place a page is listed: M253 Workspace tree (via a new `VaultFile.icon` + `SideItem.emojiIcon` slot), M254 Favorites + Recent sidebar lists, M255 command palette page rows (via a new `PageSearchResult.emojiIcon`), M256 right-rail "Linked from" backlink cards (via a new `Backlink.emojiIcon`), M257 standalone `[[ULID]]` sub-page cards in the editor body. Asset / URL icons stay invisible across all five surfaces since a 12–16px row can't render an async-loaded image.

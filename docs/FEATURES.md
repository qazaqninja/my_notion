...
> Reconciliation of every Notion feature against what Quill has actually shipped (M0–M1891+), what is partially done, what is queued, and what is explicitly out of scope. The goal is that any agent landing in this repo for the first time can open this file, pick an item, follow the file pointers it includes, and ship it as the next milestone (Mxxxx) without re-discovering the codebase.
...
4. Ship as the next milestone commit `Mxxxx: <feature>`, matching the style of the existing 1,891+ milestones (commits use 4-digit milestone IDs once the count crosses M1000).

## Status legend

- ✅ **Shipped** — landed somewhere in M0–M1891+. Commit hash in parens when notable.
...
- ✅ Breadcrumb navigation — Inline `[breadcrumb]` block renderer splits `widget.relativePath` on `/` at `lib/features/editor/presentation/widgets/markdown_renderer.dart:742` (`_renderBreadcrumb`). The original M5 `PageHeader` placement folded into `markdown_renderer.dart` when the D30d cutover deleted the legacy editor page source at M1765.
...
- ✅ Share sheet integration (save to Notion from any app) — In-app Quick capture analogue M210 (⌘. opens a dialog → prepends to `Inbox/Quick capture.md` with a timestamp). **Outbound** OS share-sheet shipped at M1240 (`share_plus`, editor-kebab "Share…" routes current page through OS sheet). **Inbound** share (open-in-Quill from Safari etc.) shipped at F1-F3 (`receive_sharing_intent: ^1.8.1` in pubspec; `IncomingShareListener` at `lib/features/sharing/presentation/incoming_share_listener.dart`; `app.dart` wires the listener to the `VaultBloc` stream so incoming text/files splice into `Inbox/Quick capture.md`).

...
- ✅ Page icons (emoji, custom upload, Notion gallery) — M26, `lib/shared/widgets/page_icon.dart` renders frontmatter `icon:` (emoji / asset / file / URL). Emoji picker (M62) at `lib/shared/widgets/emoji_picker.dart`. Sidebar row icon (M84) shows the same glyph in db row cells. M253 also surfaces page emoji icons in the Workspace tree — VaultFile gains an `icon` field populated from `frontmatter_json` during `_buildTree`, and SideItem has a new `emojiIcon` slot that draws the glyph in place of the generic `file-md` icon. Asset / URL icons stay tree-invisible since they'd need an async image fetch. M254 extends the same treatment to the sidebar's Favorites + Recent lists. M255 reaches the command palette — `PageSearchResult` gains an optional `emojiIcon` populated from `frontmatter_json` in both the FTS hit path and the path-prefix walk, and the palette row swaps the `file-md` glyph for the emoji when present.

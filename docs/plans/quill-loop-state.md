...
- **Task:** TS-01 sweep slice 5 — widget tests for `lib/features/settings/presentation/widgets/forms_pane.dart` (FormsPane + private _FormBearingPageRow + _FormsEmptyState + _FormsErrorRow). Consumes FormBearingPagesCubit (needs MockCubit pattern from bloc_test) + SyncBloc (auth gate on row tap) + FormsRepository (dialog seeding). Coverage targets: (1) BlocProvider creates cubit with .load() called — assert via mock spy; (2) loading status shows spinner; (3) failure renders _FormsErrorRow with the typed error message + a Retry button that calls cubit.load() again; (4) success+empty renders the _FormsEmptyState copy; (5) success+rows renders one _FormBearingPageRow per FormBearingPage with title, relpath, monospace formsRef chip; (6) row tap when unauthed → toastWarn "Not logged in" (no dialog); (7) row tap when authed → showDialog<FormSubmissionsDialog> opens with the page's ulid. Use record-returning pumpHelper pattern from M1441 sync_pane fix-forward. Path: `test/features/settings/widgets/forms_pane_test.dart`.
- **Status:** pending
- **Backlog (carried forward — surfaced by M1437):** "WorkspaceNameField double-commit on Enter" — pressing Enter fires _commit twice (onSubmitted + focus-loss listener since done dismisses focus). Latent bug, low impact (double file write + double RefreshFromDisk dispatch), out-of-scope for TS-01 sweep slice 3. Add a single-shot guard (`_committing` boolean) when next touched.

...
- **Task:** TS-01 sweep slice 4 — widget tests for `lib/features/settings/presentation/widgets/sync_pane.dart` (SyncPane + nested _SyncLoginCard). Consumes SyncBloc + VaultBloc — needs mocktail mocks for both. Coverage targets: login card renders email/password/login button when unauthed; submitting credentials dispatches SyncLoginRequested with the entered email+password; loading-state spinner appears during pending login; failure state renders error message; connected-state renders SyncConnectedCard (already tested as a unit — assert the higher-level wiring is right); "Push all unsynced" button is enabled when the vault is loaded + dispatches SyncPushAllRequested via the M1424 inline context.read. Path: `test/features/settings/widgets/sync_pane_test.dart`. Use the QuillToastHost + MultiBlocProvider pattern from sync_connected_card_test + the new workspace_controls_test. Will need to mock collectBulkPushEntries somehow (consider stubbing the VaultLoaded.tree to an empty tree so the entries list is empty — the dispatch should still fire even with zero entries).
- **Status:** pending

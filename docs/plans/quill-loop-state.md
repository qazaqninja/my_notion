...
- **Task:** E59 pick-next survey — E59-b PRESENCE is now end-to-end wired across all H4 slices. Live verification requires the deferred G4 docker-compose smoke (two browser sessions on the same page editing concurrently → peer cursors visible). With presence done, the next architectural lift candidates are: (a) **G4 live smoke** — boot the backend in docker-compose and walk the E2E presence + sync flow end-to-end (still pending — high value because it's the gate that proves all of H4a–H4d-iii-d-iii actually compose correctly over a real network). (b) **y_crdt WASM swap** — replace the M1452-deferred placeholder QuillCrdtDoc with a real y_crdt Dart binding. Currently the CRDT layer uses `QuillCrdtSetBody(payload)` last-writer-wins; switching to real Yjs unlocks proper concurrent edit merge without conflict toasts. (c) **Web clipper** — browser extension targeting Chrome/Firefox WebExtension API, POSTs to /sync/push. (d) **Forms polish** — typed schemas / additional .database.yaml field types in the public form renderer (M1413 + E58 series stops at unfiltered DriftFormBearingPagesRepository). (e) **TS-01 editor sweep** — add the deferred SourceView widget-level tests (would unblock all SourceView refactors). Recommend (a) as the next 1m iteration — proves the entire E59-b chain works in a real environment before adding the next feature on top of an unverified stack.
- **Status:** pending

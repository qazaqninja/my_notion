...
- **Task:** TS-01 sweep — widget tests for the 5 FS-04 extracted files. Orchestrator queued at the close of FS-04 (M1430). Coverage targets: forms_pane (loading / failure-retry / success-empty / success-rows + tap → FormSubmissionsDialog), sync_pane (login/connected/error branches + login form submit), settings_nav (active row + onSelect callback), workspace_controls (emoji-tap + name-commit dispatches RefreshFromDisk), settings_atoms (each atom renders + state transitions: SettingField.monospace, SettingButton primary/hover, SettingToggle on, SyncPlaceholderCard active). Use mockingjay/mocktail for VaultBloc + SyncBloc + repository stubs. Recommend starting with settings_atoms because the widgets are smallest + state-free, then working up to the panes. Alternative: skip TS-01 and execute E59 pick-next #3 for the next architectural lift.
- **Status:** pending

...
- **Task:** D24a — block reorder via ⌘+Shift+ArrowUp/Down in WYSIWYG. M1567 pivot survey ruled out (a) drag handle plugin opt-in (super_editor 0.3.0-dev.51 has NO built-in block-reorder plugin — `SuperEditorPlugin` subclasses are ChatPreviewMode/PatternTag/StableTag/ActionTags/MarkdownInlineUpstream; none are block drag handles) and (b) multi-select via plugin (no built-in either; `selectAll` works on Cmd+A but Notion-style click-add doesn't ship). Pivoted to D24a as the smallest-bite useful next slice: super_editor SHIPS `MoveNodeRequest` (multi_node_editing.dart:591) + `MoveNodeCommand` already wired into createDefaultDocumentEditor (default_document_editor.dart:182). All we need is a keyboard handler that resolves the current text-node + dispatches `MoveNodeRequest(nodeId, newIndex)`. Source-mode equivalent: `moveLineUp`/`moveLineDown` helpers at `lib/features/editor/domain/source_line_ops.dart:35,61` — same TDD shape as the D26 detector+reaction slices. Slice 1: pure-Dart resolver returning `({String nodeId, int newIndex})?` given (document, selection, direction). Slice 2: super_editor `ExecutionInstruction` keyboard handler wiring + ⌘+Shift+ArrowUp/Down binding.
- **Status:** pending

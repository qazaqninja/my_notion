...
- **Task:** E59 — pick-next survey #3: Phase E forms work is end-to-end live (M1401 backend renderer → M1403 GET route → M1406 editor kebab + endpoint constants → M1418 Settings pane). Roadmap candidates: (a) y_crdt WASM swap — biggest single-feature lift; pulls wasm_run + wasm_wit_component into client and backend; replaces opaque-text wire with binary CRDT updates. (b) Yjs awareness for cursor presence on the same WS. (c) Phase E E59-E60 forms polish: server-side iframe-friendly embed route (no chrome, no submit button text — just the form), url_launcher "Open form preview" kebab action. (d) Phase E E61+ web clipper extension. (e) Long-tail debt: FS-04 settings_page.dart decomposition (now 2,230 lines with 11 private widget classes), TS-01 widget tests for E58b-iii / E57 / G2.5 wire-ins, TS-07 coverage ratchet, MD-01 DTO/state split. Recommendation: (e) FS-04 decomposition first — the file size is past threshold and the orchestrator flagged it. Then a pick-next for the next architectural lift.
- **Status:** pending

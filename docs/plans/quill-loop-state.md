...
- **Task:** E58b-ii — Settings → Forms pane UI on top of the M1412/M1413 data layer. Add `'forms'` to _SettingsPageState's nav rail (under the "Data" group alongside Export & Backup), `_activeLabel`, and `_content` switch; render a FutureBuilder-driven list of FormBearingPage rows from `context.read<FormBearingPagesRepository>().loadAll()` with each row tapping into the existing FormSubmissionsDialog (E51). Empty-state copy + a Cubit for loading state if the FutureBuilder pattern feels too thin. Wire the DriftFormBearingPagesRepository into app.dart's RepositoryProvider.
- **Status:** pending

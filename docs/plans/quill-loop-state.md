...
- **Task:** E59 — pick-next #3 survey (post-TS-01-sweep). With FS-04 decomposition + TS-01 widget tests both complete, the next architectural lift candidates are: (a) **WorkspaceNameField double-commit fix** — small, surfaced as backlog from M1437 — single-shot `_committing` guard before _commit's async I/O. (b) **y_crdt WASM swap** — replace the placeholder QuillCrdtDoc with a real y_crdt Dart binding, lights up CRDT-level concurrent edits. (c) **Presence over WebSocket** — onstage cursors via the existing SyncWsClient channel. (d) **Web clipper** — browser extension targeting Chrome/Firefox WebExtension API, POSTs to `/sync/push`. (e) **Forms polish** — typed schemas / additional .database.yaml field types in the public form renderer. Recommend (a) as the next 1m iteration (surgical fix surfaced by the same sweep that just closed) then resurvey for the next lift.
- **Status:** pending

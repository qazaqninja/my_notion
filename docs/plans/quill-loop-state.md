...
- **Task:** H3b — pick-next survey #2: extractions / decompositions / deferred polish complete (H2 multiplayer end-to-end, BL-12 toast throttle, FindInPageController extracted). Next architectural choices: (a) H3 y_crdt WASM swap (replace opaque-text wire format with binary CRDT updates — biggest single-feature lift; pulls wasm_run + wasm_wit_component into both client and backend), (b) presence/cursor awareness on the same WS (Yjs awareness channel showing peers' cursors — natural next slice after the wire-format swap, but can ship over the text wire too), (c) Phase E E56-E65 forms work (form designer UI in Settings, form public/embed routes), (d) Phase A/F long-tail debt (TS-07 coverage ratchet to 95%, FS-03 packages/ restructure, MD-01 DTO/state split). Recommend (c) Phase E forms — concrete user-visible feature with a clean boundary, doesn't depend on H2 work.
- **Status:** pending

...
- **Phase:** D (super_editor WYSIWYG migration) — **D24-D30 pivot done**
- **Task:** D26 bold autoformat slice 1 — pure-Dart pattern detector. Survey at M1548 found that super_editor's `defaultEditorReactions` already cover ALL the block-prefix conversions D26 was originally scoped to port (HeaderConversion / UnorderedListItem / OrderedListItem / Blockquote / HorizontalRule / ImageUrl / Dash). Quill's slash menu duplicates those reactions but via a different trigger (`/`), so source-mode's M234-M268 block-prefix shortcuts are effectively already in WYSIWYG. The real user-facing gap is **inline mark autoformat** — typing `**bold**` in WYSIWYG should auto-toggle the bold attribution on the captured span and strip the markers. First TDD slice: a pure-Dart helper `detectBoldAutoformat(text, caret) → ({markerStart, markerEnd, inner})?` that returns a non-null span when the keystroke just completed a `**X**` pattern. Real super_editor `EditReaction` wiring lands in a follow-up slice. D24 (drag handles) + D25 (multi-select) deferred — both are likely thin wrappers over super_editor's built-in plugins. D28-D30 cutover stays last (beta needs to prove itself).
- **Status:** pending

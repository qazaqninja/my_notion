...
- **Phase:** D (super_editor WYSIWYG migration) — **D26 closed (7/7 inline marks, 148 tests). D24a slice 1 (resolver) shipped. Slice 2 (keyboard wiring) pending.**
- **Task:** D24a slice 2 — wire `resolveBlockReorder` into a super_editor `ExecutionInstruction` keyboard handler bound to ⌘+Shift+ArrowUp / ⌘+Shift+ArrowDown. Dispatches `MoveNodeRequest(nodeId, newIndex)` (built-in command at multi_node_editing.dart:591+601, wired by createDefaultDocumentEditor at default_document_editor.dart:182). On macOS use Cmd; on Linux/Windows use Ctrl. Place the handler at `lib/features/editor/presentation/controllers/block_reorder_keyboard_action.dart`; register into `editor_beta_page.dart` via `SuperEditor.keyboardActions` (prepend, so block-reorder happens before super_editor's default selection-move). Test via widget test that pumps a SuperEditor with 3 paragraph nodes, places caret in middle node, sends ⌘+Shift+Up via `KeyboardListener.maybeOf`-style harness — assert document.getNodeAt(0) is now the moved node. Or simpler: dispatch a synthetic `KeyEvent` through a `_RecordingDispatcher`-style harness if the widget test friction is high. After this slice: D24a closes; backlog re-prioritises (D24b indent/outdent via ⌘+[/⌘+] is the natural follow-on; then D25 multi-select).
- **Status:** pending

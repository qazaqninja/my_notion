...
- **Task:** E59-b H4d-iii — close the presence loop by wiring `RemoteCursorOverlay` (M1454) into the editor body + connecting both presence directions to the live SyncWsBinder. Three concerns to land in this slice (or sub-slice if too large): (1) `EditorSyncWsMount` (or its parent _EditorBodyState) creates a per-page `PresenceCubit`, subscribes to `binder.awarenessStream` and forwards each AwarenessMessage to `cubit.remoteCursorReceived(msg)`, and disposes the cubit when the editor page tears down. (2) A `cursorRectAt` resolver wired to the live `RenderEditable` so the M1454 chiclets paint at real caret positions — likely a `getRectForCaret(TextPosition(offset: idx))` lookup with bounds-checking. (3) Local TextField selection changes (after the M1437-style debounce) emit `binder.sendAwareness(AwarenessMessage(userId: syncBloc.state.userId or derived, pageUlid: ulid, cursorIndex: selection.baseOffset, color: peerColorFromUserId(userId)))`. This is the largest of the H4d slices; consider further sub-bites if scope grows: H4d-iii-a (overlay + cubit mount), H4d-iii-b (rect resolver), H4d-iii-c (debounced outbound).
- **Status:** pending

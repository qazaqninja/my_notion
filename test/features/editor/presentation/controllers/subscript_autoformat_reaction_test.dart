...
import 'package:my_notion/features/editor/presentation/controllers/subscript_autoformat_reaction.dart';
import 'package:super_editor/super_editor.dart';

void main() {
  group('SubscriptAutoformatReaction.react', () {
    ...
    const reaction = SubscriptAutoformatReaction();

    setUp(() {
      document = MutableDocument(
        nodes: [
          ParagraphNode(id: 'p1', text: AttributedText('~h2o~')),
        ],
      );
    ...
    group('guards (no-op paths)', () {
      ...
      test('no-op when caret is mid-span (no closing tilde yet)', () {
        composer.setSelectionWithReason(
          const DocumentSelection.collapsed(
            position: DocumentPosition(
              nodeId: 'p1',
    ...
    group('dispatch (positive path)', () {
      test('dispatches DeleteContentRequest + InsertTextRequest with '
          'subscriptAttribution when caret is at end of `~h2o~`', () {
        composer.setSelectionWithReason(
          const DocumentSelection.collapsed(
            position: DocumentPosition(
              nodeId: 'p1',
        ...
        expect(insert.textToInsert, 'h2o');
        expect(insert.attributions, contains(subscriptAttribution));
      });
    });
  });
}

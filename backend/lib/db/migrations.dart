...
const _migrations = <_Migration>[
  ...
  '''),
  _Migration(6, '''
    -- E48 (M1351): per-submission storage.
    -- Each POST /forms/<ulid>/submit writes one row here with the
    -- parsed form fields as JSONB. `page_ulid` is the source page
    -- (foreign key not enforced — pages can be deleted from the
    -- mirror while preserving submission history). `source_ip` is
    -- best-effort; missing-or-proxy hides behind the connection
    -- info shelf already exposes.
    CREATE TABLE form_submissions (
      id          TEXT PRIMARY KEY,            -- ULID generated by the route handler
      page_ulid   TEXT NOT NULL,
      fields      JSONB NOT NULL,
      source_ip   TEXT,
      created_at  TIMESTAMPTZ NOT NULL DEFAULT now()
    );
    CREATE INDEX idx_form_submissions_page
      ON form_submissions(page_ulid, created_at DESC);
  '''),
];
